...
//$image_extnation=end(explode(".",$image_name));
        ...
        if(empty($image_name)/*!in_array($image_extnation,$image_allow_extnation)*/){
            $formeror[]='please inter onther image';
        }
        if(empty($name)){
...
<h2 class="inner-tittle">update post </h2>
    ...
    <label for="exampleInputEmail1">price</label> 
    <input type="number" class="form-control" id="exampleInputEmail1" name="price"  value=<?php echo$row['price']?> placeholder="price"> </div>
...
//$image_extnation=end(explode(".",$image_name));
    ...
    $fohmeror[]='please inter name';
        ...
        if(empty($formeror)){
            $image=rand(0,10000000).'_'.$image_name;
            move_uploaded_file($image_tmp,"../upload\avatar\\".$image);
            $query="UPDATE post SET name='$name',descrabtion='$des',price='$price',image='$image',cat_id='$cat',user_id='$user'
            WHERE id='$id'";
        $res=mysqli_query($con,$query);
        if(isset($res)){
            ?>
